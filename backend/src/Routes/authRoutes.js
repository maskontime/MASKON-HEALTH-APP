...
router.post('/register', [
    ...
    try {
        ...
        if (personnel) return res.status(400).json({ success: false, error: 'Personnel already exists' });

        personnel = await Personnel.create(req.body);

        const token = jwt.sign({ id: personnel._id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE });
...
router.post('/login', [
    ...
    try {
        ...
        if (!personnel) return res.status(401).json({ success: false, error: 'Invalid credentials' });

        const isMatch = await personnel.matchPassword(password);
        if (!isMatch) return res.status(401).json({ success: false, error: 'Invalid credentials' });

        const token = jwt.sign({ id: personnel._id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE });
        res.status(200).json({ success: true, token, data: { id: personnel._id, name: personnel.name, email: personnel.email, role: personnel.role } });
    } catch (err) {
        res.status(500).json({ success: false, error: 'Server Error' });
    }
});
